...
usage() {
  cat <<'EOF'
Usage:
  start_slice.sh [--root /path/to/workspace] [--from-next] [--milestone-id id] [--milestone-title title] [--slice-id id] [--slice-title title] [--next-slice text] [--summary text] <task-slug>
...
export CHECKPOINT_FILE HANDOFF_FILE PROGRESS_FILE ARTIFACTS_FILE TASK_SLUG FROM_NEXT MILESTONE_ID MILESTONE_TITLE SLICE_ID SLICE_TITLE NEXT_SLICE SUMMARY_TEXT
...
if not resume_prompt:
    if execution.get("mode") == "rolling":
        if execution.get("run_style") == "until_stopped":
            resume_prompt = (
                f"Use $task-framer, $course-corrector, $long-horizon-worker, $task-evaluator, "
                f"and $task-handoff-state to continue .autonomous/{task_slug}/. Keep planning bounded, "
                "auto-advance through queued milestones, replenish the queue when policy allows, and keep "
                "going until a real blocker, decision boundary, safe-work exhaustion, or an external manual "
                "pause appears. Do not impose an internal runtime budget or milestone cap."
            )
        else:
            resume_prompt = (
                f"Use $task-framer, $course-corrector, $long-horizon-worker, $task-evaluator, "
                f"and $task-handoff-state to continue .autonomous/{task_slug}/. Keep planning bounded, "
                "auto-advance through queued milestones, replenish the queue when policy allows and budget "
                "remains, do not self-pause before the configured minimum runtime and milestone floor unless "
                "a blocker or decision boundary appears, and stop only on the configured run limits or a real "
                "blocker."
            )
    else:
        resume_prompt = (
            f"Use $long-horizon-worker and $task-handoff-state to continue .autonomous/{task_slug}/ from the "
            "current handoff and validate the next slice before stopping."
        )

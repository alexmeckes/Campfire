...
trap 'rm -rf "$TEMP_WORKSPACE" /tmp/campfire_codex_hook_continue.out /tmp/campfire_codex_hook_waiting.out /tmp/campfire_codex_hook_reentrant.out' EXIT
...
cp "$ROOT_DIR/.codex/hooks/campfire-stop.sh" "$TEMP_WORKSPACE/.codex/hooks/"
...
echo "PASS: Codex Stop hook verification completed."

...
- `resume`
  ...
  - For rolling Codex runs, the rendered prompt includes the `$thread-monitor-sidecar` instruction plus the repo-local `./scripts/monitor_task_loop.sh <task-slug>` command that the reused sidecar should target.
- `rolling_resume`
